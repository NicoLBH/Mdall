...
export async function fetchGoogleMapsPlaceEmbedUrl({ latitude = null, longitude = null, zoom = 14, mapType = "roadmap" } = {}) {
  ...
  const response = await fetch(`${SUPABASE_URL}/functions/v1/google-maps-embed-url`, {
    method: "POST",
    headers: await buildSupabaseAuthHeaders({ "Content-Type": "application/json", Accept: "application/json" }),
  ...
  if (!response.ok) {
    const details = await response.text().catch(() => "");
    const error = new Error(`google-maps-embed-url failed (${response.status}): ${details}`);
    console.error("[google-maps-embed] url.fetch.failure", { latitude: lat, longitude: lon, zoom: safeZoom, mapType: safeMapType, message: error.message });
    throw error;
  }
