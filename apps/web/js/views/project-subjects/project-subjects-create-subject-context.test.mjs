...
test("Créer un sous-sujet ouvre le create form en mode subissue (modale)", () => {
  ...
  assert.match(viewSource, /\$\{isSubissueMode \? `<div class="subject-create-inline-meta">\$\{renderCreateSubjectMetaControls\(\)\}<\/div>` : ""\}/);
  assert.match(viewSource, /\$\{isSubissueMode \? "" : renderCreateSubjectFooterHtml\(\{ form, isSubissueMode \}\)\}/);
  assert.match(viewSource, /setSubissueModalScrollLock\(isSubissueCreateMode\);/);
  assert.match(viewSource, /function setSubissueModalScrollLock\(isLocked\)/);
  assert.match(viewSource, /document\.body\.style\.position = "fixed";/);
  assert.match(viewSource, /window\.scrollTo\(0, scrollY\);/);
  ...
  assert.match(styleSource, /\.settings-modal__dialog\.subject-create-subissue-modal__dialog\{[\s\S]*display:flex;[\s\S]*flex-direction:column;/);
  ...
  assert.match(styleSource, /\.subject-create-subissue-modal__dialog \.settings-modal__head\{[\s\S]*height:49px;[\s\S]*padding:8px 8px 8px 16px;/);
  assert.match(styleSource, /\.subject-create-subissue-modal__body\{[\s\S]*flex:1 1 auto;[\s\S]*min-height:0;[\s\S]*padding:16px;[\s\S]*overflow-y:auto;[\s\S]*overscroll-behavior:contain;/);
  ...
  assert.match(styleSource, /\.subject-create-layout--subissue \.subject-create-inline-meta\{[\s\S]*margin-top:12px;/);
