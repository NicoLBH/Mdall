    ...
    const bodyHtml = visibleObjectives.length
      ? visibleObjectives.map((objective) => {
          ...
          <div class="objectives-row" data-objective-id="${escapeHtml(objective.id)}" tabindex="0" role="button">
            ...
            <span class="objectives-row__meta">${overdue ? `<span class="objectives-row__overdue"><span class="objectives-row__overdue-icon" aria-hidden="true">${svgIcon("alert-fill", { className: "octicon octicon-alert-fill" })}</span><span>${escapeHtml(overdue.label)}</span></span><span aria-hidden="true">•</span>` : ""}Échéance au ${escapeHtml(dueDateLabel)} <span aria-hidden="true">-</span> ${renderProblemsCountsIconHtml(counts.closed, counts.total)}</span>
