...
test("le dragstart est armé par pointerdown sur le handle et utilise un drag preview dédié", () => {
  ...
  assert.match(eventsSource, /dragPreviewNode\.style\.backgroundColor = previewBackgroundColor;/);
  assert.match(eventsSource, /dragPreviewNode\.style\.borderStyle = "solid";/);
  assert.match(eventsSource, /dragPreviewNode\.style\.borderWidth = "1px";/);
  assert.match(eventsSource, /dragPreviewNode\.style\.borderColor = previewBorderColor;/);
  assert.match(eventsSource, /dragPreviewNode\.style\.borderRadius = previewBorderRadius;/);
  assert.match(eventsSource, /dragPreviewNode\.style\.boxShadow = "0 14px 36px rgba\(1,4,9,.55\), 0 0 0 1px rgba\(1,4,9,.35\)";/);
  assert.match(eventsSource, /borderStyle: dragPreviewNode\.style\.borderStyle,/);
  assert.match(eventsSource, /borderWidth: dragPreviewNode\.style\.borderWidth,/);
  assert.match(eventsSource, /borderColor: dragPreviewNode\.style\.borderColor,/);
  assert.match(eventsSource, /boxShadow: dragPreviewNode\.style\.boxShadow,/);
