...
const SOURCE_PATH='apps/web/js/services/zoning/snow-canton-regions.json';
const OUTPUT_PATH='supabase/seed-data/climate/005_snow_canton_overrides.sql';
console.log('[climate-import] snow-canton-overrides start');
const rows=[];
...
const sql=buildInsertStatement({tableName:'public.mdall_climate_snow_canton_overrides',columns:['department_code','canton_name','canton_name_normalized','resolved_zone','source_payload'],rows,onConflictClause:`ON CONFLICT (department_code, canton_name_normalized) DO UPDATE\nSET canton_name = EXCLUDED.canton_name,\n    resolved_zone = EXCLUDED.resolved_zone,\n    source_payload = EXCLUDED.source_payload,\n    updated_at = now()`,useRawColumns:new Set(['source_payload'])});
await writeSqlFile(OUTPUT_PATH,`-- Generated from ${SOURCE_PATH}\n${sql}`);
console.log('[climate-import] snow-canton-overrides written');
