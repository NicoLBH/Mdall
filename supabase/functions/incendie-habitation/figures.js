 ...
 */
function bande({ x, y, largeur, hauteur, intitule, colonnes, marques, groupes = true, motsEnTete = false }) {
  ...
  colonnes.forEach((c, i) => {
    ...
    t.push(`<line x1="${rail(i)}" y1="${y + hEntete}" x2="${rail(i)}" y2="${y + hauteur}" class="fig-rail"/>`);
    if (i > 0) {
  ...
  t.push(`<line x1="${x - 48}" y1="${yRonds}" x2="${x + largeur}" y2="${yRonds}" class="fig-appel"/>`);
