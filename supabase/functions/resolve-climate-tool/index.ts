...
async function resolveZonedClimate(params: any) {
  const { supabase, toolKey, codeInsee, departmentCode, cantonCode2014, cantonName2014, cantonNameCurrent, cityName } = params;
  ...
  console.log("[resolve-climate-tool] zone.resolve.start", { toolKey, codeInsee, departmentCode, cantonCode2014, cantonName2014, cantonNameCurrent, cityName, matchType: null });
  ...
  const cityAllCantonsCandidates = buildAllCantonsCandidates(cityName);
  ...
  for (const candidate of cityAllCantonsCandidates) {
    const { data: override } = await supabase
      .from(overridesTable)
      .select("resolved_zone")
      .eq("department_code", departmentCode)
      ...
      .maybeSingle();
    if (override?.resolved_zone) {
      console.log("[resolve-climate-tool] zone.resolve.result", { toolKey, codeInsee, departmentCode, cantonCode2014, cantonName2014, cantonNameCurrent, cityName, matchType: candidate.matchType });
      return { zone: override.resolved_zone, debug: { match_type: candidate.matchType, matched_key: candidate.value } };
    }
  }
  ...
  for (const candidate of otherCandidates) {
    const { data: override } = await supabase
      ...
      .maybeSingle();
    if (override?.resolved_zone) {
      console.log("[resolve-climate-tool] zone.resolve.result", { toolKey, codeInsee, departmentCode, cantonCode2014, cantonName2014, cantonNameCurrent, cityName, matchType: "fallback_other_cantons" });
      return { zone: override.resolved_zone, debug: { match_type: "fallback_other_cantons", matched_key: candidate } };
    }
  }

  console.error("[resolve-climate-tool] zone.resolve.failure", { toolKey, codeInsee, departmentCode, cantonCode2014, cantonName2014, cantonNameCurrent, cityName, matchType: "cantons_required" });
  throw new HttpError(400, JSON.stringify({
    ...
    debug: { department_code: departmentCode, canton_code_2014: cantonCode2014, canton_name_2014: cantonName2014, canton_name_current: cantonNameCurrent, city_name: cityName, department_zone_count: departmentRows.length }
  }));
}

function buildNameCandidates(cantonName2014: string | null, cantonNameCurrent: string | null) {
  const hasRegulatoryCantonName = Boolean(cantonName2014 && !isUnspecifiedCantonName(cantonName2014));
  const all = [
    { name: hasRegulatoryCantonName ? cantonName2014 : null, prefix: "canton_2014" },
    // A current canton must not override the regulatory 2014 mapping when the 2014
    // reference only says "Canton non précisé". In these cases, use the commune
    // name to match "<ville> (tous cantons)" before falling back.
    { name: hasRegulatoryCantonName ? cantonNameCurrent : null, prefix: "canton_current" }
  ];
...
function buildAllCantonsCandidates(cityName: string | null) {
  if (!cityName) return [];
  const base = String(cityName).trim();
  if (!base || isUnspecifiedCantonName(base)) return [];

  const labels = [
    `${base} (tous cantons)`,
    `${base.replace(/^Le\s+/i, "")} (tous cantons)`,
    `${base.replace(/^La\s+/i, "")} (tous cantons)`,
    `${base.replace(/^Les\s+/i, "")} (tous cantons)`
  ];

  const result: Array<{ value: string; matchType: string }> = [];
  for (const label of labels) {
    const values = [normalizeLegacyName(label), normalizeFrontName(label), normalizeMinimalName(label)];
    for (const value of values) {
      if (value && !result.some((x) => x.value === value)) {
        result.push({ value, matchType: `city_all_cantons_${value}` });
      }
    }
  }
  return result;
...
function isUnspecifiedCantonName(value: string | null) {
  if (!value) return false;
  return normalizeFrontName(value) === "canton non precise";
}
