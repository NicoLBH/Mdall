...
serve(async (req) => {
  try {
    ...
    if (!parsed && !llmText) {
      ...
      return jsonResponse(
        { error: "No structured or text output returned by OpenAI", raw: openAiJson },
        502
      );
    }
...
function parseLlmResponse(payload: any, llmText: string | null): LlmResponse | null {
  const directParsed = extractParsedObjectFromOpenAiResponse(payload);
  if (isLlmResponseShape(directParsed)) {
    return directParsed;
  }

  const textCandidates = [
    llmText,
    ...extractTextCandidatesFromOpenAiResponse(payload)
  ].filter((value): value is string => typeof value === "string" && value.trim().length > 0);

  for (const candidate of textCandidates) {
    const parsed = tryParseJsonCandidate(candidate);
    if (isLlmResponseShape(parsed)) {
      return parsed;
    }
  }
...
function extractParsedObjectFromOpenAiResponse(payload: any): unknown {
  ...
  for (const item of payload.output) {
    ...
    for (const contentItem of item.content) {
      if (contentItem && typeof contentItem.parsed === "object" && contentItem.parsed !== null) {
        return contentItem.parsed;
      }

      if (contentItem && typeof contentItem.json === "object" && contentItem.json !== null) {
...
function extractTextCandidatesFromOpenAiResponse(payload: any): string[] {
...
function tryParseJsonCandidate(value: string): unknown {
  ...
  const withoutFence = unwrapCodeFence(trimmed);
  if (withoutFence !== trimmed) {
    const parsed = safeJsonParse(withoutFence);
    if (parsed !== null) {
      return parsed;
    }
  }
